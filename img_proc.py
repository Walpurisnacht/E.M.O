...
for i in range(len(limg)):
    ...
    #Save point
    cimg = cname.entryList(["*.jpg","*.png"], QDir.Files, QDir.Name)
    with open(clist + str(cimg[i])[:-4] + ".pts",'w') as fout:
        data = "version: 1\nn_points: 68\n}\n"
        for x in range(68):
            data += str(raw[x][0])
            data += " "
...
"""
lines = open(flist+lann[0]).read().split("\n")
lines = lines[3:71]
...
#cropped = cv2.imread(clist+cimg[0])
